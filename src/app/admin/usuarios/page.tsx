...
export default function UsersPage() {
  ...
  return <>
    ...
    <div className="user-kpis">{[[users.length,"Total usuarios"],[users.filter((u)=>u.role==="ADMIN").length,"Administradores"],[users.filter((u)=>u.role==="SUPERVISOR").length,"Supervisores de visitas"],[users.filter((u)=>u.role==="INCIDENT_SUPERVISOR").length,"Supervisores de incidencias"],[users.filter((u)=>u.status==="ACTIVE").length,"Activos"]].map(([number,label])=><article key={String(label)}><b>{number}</b><span>{label}</span></article>)}</div>
    <section className="card user-filters"><label><Search /><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Buscar nombre, correo o teléfono..." /></label><select value={role} onChange={(e)=>setRole(e.target.value)}><option value="ALL">Todos los roles</option><option value="ADMIN">Administrador</option><option value="SUPERVISOR">Supervisor de visitas</option><option value="INCIDENT_SUPERVISOR">Supervisor de incidencias</option></select><select value={status} onChange={(e)=>setStatus(e.target.value)}><option value="ALL">Todos los estados</option>{["ACTIVE","INACTIVE","SUSPENDED","PENDING"].map((value)=><option key={value}>{value}</option>)}</select></section>
